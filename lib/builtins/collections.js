...
const callCallable = (callable, inp, args = []) => Promise.resolve(callable(inp, args));
const list = (0, helpers_1.command)((inp, args) => args);
const obj = (0, helpers_1.command)((inp, args) => buildObjectFromArgs(args));
const range = (0, helpers_1.command)((inp, args) => {
    let start;
    let end;
    if (args.length === 2) {
...
const length = (0, helpers_1.command)((inp) => inp.length);
const reverse = (0, helpers_1.command)((inp) => {
    if (typeof inp === "string") {
        return inp.split("").reverse().join("");
    }
    return inp.slice().reverse();
});
const head = (0, helpers_1.command)((inp, args) => {
    const count = args[0] || 50;
    return inp.slice(0, count);
});
const tail = (0, helpers_1.command)((inp, args) => {
    const count = args[0] || 50;
    if (count < 1) {
        return [];
    }
...
const join = (0, helpers_1.command)((inp, args) => inp.join(args[0] || " "));
const split = (0, helpers_1.command)((inp, args) => inp.split(args[0] || "\n"));
const match = (0, helpers_1.command)((inp, args) => {
    let source = inp;
    let pattern = args[0];
    if (args.length === 2) {
        source = args[0];
...
const filter = (0, helpers_1.command)(async (inp, args, ctx) => {
    const callable = (0, common_1.resolveCallable)(args[0], ctx);
    if (callable) {
        const rest = args.slice(1);
        const keeps = await Promise.all(inp.map((item) => callCallable(callable, item, rest)));
...
const map = (0, helpers_1.command)(async (inp, args, ctx) => {
    const callable = (0, common_1.resolveCallable)(args[0], ctx);
    if (!callable) {
...
const each = (0, helpers_1.command)(async (inp, args, ctx) => {
    const callable = (0, common_1.resolveCallable)(args[0], ctx);
    if (!callable) {
...
const reduce = (0, helpers_1.command)(async (inp, args, ctx) => {
    const callable = (0, common_1.resolveCallable)(args[0], ctx);
    if (!callable) {
...
const lambdaKeys = (inp, lam) => Promise.all(inp.map((item) => lam.call(item, [])));
const sort = (0, helpers_1.command)((inp, args) => {
...
const count = (0, helpers_1.command)((inp, args) => {
...
const bin = (0, helpers_1.command)((inp, args) => {
...
const chunks = (0, helpers_1.command)((inp, args) => {
...
const slice = (0, helpers_1.command)((inp, args) => {
    const start = args[0] || 0;
    const end = args[1] || inp.length;
    return inp.slice(start, end);
});
const zip = (0, helpers_1.command)((inp, args) => zipArgs(args));
const zipobj = (0, helpers_1.command)((inp, args) => {
...
collectionsBuiltins.sortBy = (0, helpers_1.command)(async (inp, args) => {
...
collectionsBuiltins.groupBy = (0, helpers_1.command)(async (inp, args) => {
    if (args[0] instanceof helpers_1.Lambda) {
        const keys = await lambdaKeys(inp, args[0]);
        const groups = {};
        inp.forEach((item, i) => {
