...
const strip_html_cmd = (inp, args, ctx, cb) => {
    cb(null, strip_html(inp));
};
exports.strip_html_cmd = strip_html_cmd;
// "text" -> keywords -> ["keyword"]
const keywords = (inp, args, ctx, cb) => {
...
// "text" -> words -> ["word"]
const words = (inp, args, ctx, cb) => {
    cb(null, inp.match(/[\w']{2,}/g));
};
exports.words = words;
// "text" -> slugify -> "slug"
const slugify = (inp, args, ctx, cb) => {
    cb(null, inp
        .toLowerCase()
        .replace(/\W+/g, "-")
        .replace(/^\W?(.+)\W$/, "$1"));
