    ...
    const parseArgs = async (inp, args) => {
        ...
        const replaceArg = async (arg) => {
            if ((0, lang_1.isObject)(arg)) {
                ...
                const lone = masked.match(/^(?:\$\{([!a-zA-Z0-9_-]+)\}|\$([!a-zA-Z0-9_-]+))$/);
                if (lone) {
                ...
                if (masked.includes("$") || masked.includes(ESC)) {
                    ...
                    const replaced = masked.replace(/\$\{([!a-zA-Z0-9_-]+)\}|\$([!a-zA-Z0-9_-]+)/g, (_m, bracedKey, bareKey) => {
...
async function doCmd(_args, inp, ctx) {
    ...
    const invoke = async (run) => {
        try {
            ...
            if ((0, lang_1.isObject)(e) && !(e instanceof Error)) {
                throw e;
            }
...
function previewMapKey(key) {
    if ((0, lang_1.isString)(key) || typeof key === "number")
        return String(key);
    if ((0, lang_1.isObject)(key) && key.obj != null)
        return "{...}";
...
async function mapInto(l, f, d, label) {
    if (!Array.isArray(l)) {
...
function accessor(obj, key) {
    if (key === ".") {
        ...
        if (typeof obj.slice !== "function") {
            ...
            throw shapeError(`at-expression slice '${start ?? ""}..${end ?? ""}' expects an array or string`, obj);
        }
        const { start, end } = key.slice;
        ...
        if (Array.isArray(obj)) {
            throw shapeError(`at-expression reads key '${key}' from an array; use ':${key}' to map over the items or an index to pick one`, obj);
        }
        return obj[key];
    }
}
